  ...
  <div ref="proformaDetalle" v-if="item_proforma"
    ...
    <div class="bg-white p-4 rounded-md border-2 max-w-2xl w-full overflow-x-auto">
      <div class="flex justify-end">
        <button @click="$emit('close')" class="px-4 py-2 mx-2 bg-red-500 hover:bg-red-700 text-white rounded-md">X</button>
      </div>
      <!-- Contenido del modal -->
      ...
      <div class="mt-2 flex space-x-2 justify-center">
        <button @click="copiarProforma()" class="flex items-center text-black px-2 py-1 rounded border border-gray-800 bg-[#d2e062] hover:bg-[#646b2f]">
          <img src="@/assets/content_copy.svg" alt="Copiar">
          <span>Copiar</span>
        </button>
        <button @click="editarProforma()" class="flex items-center text-black px-2 py-1 rounded border mx-1 border-gray-800 bg-[#f39c12] hover:bg-[#e67e22]">
          <img src="@/assets/edite.svg" alt="Editar">
          <a href="/">Editar</a>
        </button>
        <button @click="$emit('close')" class="px-4 py-2 mx-2 bg-red-700 hover:bg-red-900 text-white rounded border border-gray-800"> X Cerrar</button>
      </div>
    </div>
  </div>
</template>
